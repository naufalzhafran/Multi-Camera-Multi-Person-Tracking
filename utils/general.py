...
def set_logging(rank=-1, verbose=True):
  logging.basicConfig(format="%(message)s",
                      ...
                      (verbose and rank in [-1, 0]) else logging.WARN)
...
def check_git_status(
    err_msg=', for updates see https://github.com/ultralytics/yolov5'):
...
def check_requirements(requirements='requirements.txt', exclude=()):
  ...
  if isinstance(requirements, (str, Path)):  # requirements.txt file
    ...
    if not file.exists():
      ...
      return
    requirements = [
        f'{x.name}{x.specifier}' for x in pkg.parse_requirements(file.open())
...
def check_file(file):
  ...
  if Path(file).is_file() or file == '':  # exists
    ...
    ) and Path(file).stat().st_size > 0, f'File download failed: {url}'  # check
    return file
  else:  # search
...
def download(url, dir='.', unzip=True, delete=True, curl=False, threads=1):
  ...
  if threads > 1:
    ...
    pool.imap(lambda x: download_one(*x), zip(url,
                                              repeat(dir)))  # multi-threaded
    pool.close()
    pool.join()
  else:
